(function() {
  ...
  //   .exponent(3)
  //   .domain([0, 15000000, 30000000, 45000000, 60000000])
  ...
  // {
  //   mass_grams
  //   reclong:  longitude_coordinates
  //   reclat:  latitude_coordinates
  //   year: timestamp_of_quake
  // }
  // {
  ...
  d3.json('Clean_Meteor_Data2.json', function(err, data) {
    if (err) {
    ...
    d3.timer(function() {
      ...
      Console.log(toPing.length);

      for (var i = 0; i < toPing.length; i++) {
        var ping = toPing[i];
        planet.plugins.pings.add(ping.reclong, ping.reclat, {
